...
for dataset in ['val2017', 'train2017']:
	...
	for i, (imgId, imgInfo) in enumerate(zip(imgIds, imgInfos)):
		...
		hm_c = np.array(np.maximum(np.minimum(hm_c, 255), 0), np.uint8).transpose([1, 2, 0])
		io.imsave('/disks/data4/zyli/Faster-RCNN-AlphaPose/heatmap/%s/%s.png' % (dataset, imgIdStr), hm_c)
